  ...
  data() {
    return {
      ...
      tableFields: [
        ...
        { label: "封面图", prop: "pic", type: "image" },
        { label: "视频大小", prop: "size" },
  ...
  methods: {
    ...
    editClass(item) {
      this.$prompt("请输入分类名称", "修改分类名称", {
        ...
        await this.axios("/adminapi/videoType/edit", {
    ...
    async delClass(item) {
      await this.axios("/adminapi/videoType/del", {
        data: { id: item.id }
      });
      this.getData();
    },
    ...
    async delArticle() {
      await this.axios("/adminapi/video/del", {
        data: { ids: this.ids }
      });
      console.log(0);
    ...
    async addClass() {
      this.$prompt("请输入分类名称", "添加新分类", {
        ...
        await this.axios("/adminapi/videoType/add", {
          data: { label: e.value }
        });
        this.getData();
      });
