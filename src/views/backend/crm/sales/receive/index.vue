...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectList: [],
      selectionData: { channel: null, source: null, status: null, channel_type: 4 },
      selectionField: [
        {
          label: '接待渠道',
          type: 'select',
  ...
  methods: {
    ...
    async selectEdit() {
      ...
      let obj = { id: this.selectList.map(e => e.id), ...this.selectionData }
      await this.axios('/adminapi/Customerrefunder/edit', { data: obj })
      this.getData(false)
    },
    ...
    async weight(item) {
      this.$prompt('请输入权重', '提示', {
        ...
        .then(async e => {
          await this.axios('/adminapi/Customerrefunder/weight', {
