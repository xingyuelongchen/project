  ...
  <div class="content">
    <div class="content-cols">
      ...
      <mixTable v-model="tableData" :fields="tableFields" @select="selection">
        <div slot="head" style="padding:10px;">
          <template v-for="(item,index) in btn">
            <el-button :type="item.style" :key="index" v-role="item.role" @click.stop="item.click()" size="mini">{{item.label}}</el-button>
          </template>
        </div>
      </mixTable>
      <mixPage v-model="page" />
    </div>
    <mixDrawer style="top:40px" v-model="drawer" :title="drawerName" @confirm="onSave" @close="drawerClose" :isShow="true">
...
export default {
    ...
    return {
      ...
      tableSales: [
        ...
        },
        {
          label: '操作',
          type: 'button',
          options: [{ label: '分配', click: this.onDistribution, style: 'primary' }]
        }
      ],
      selectList: [],
      ...
      salesSearch: [
        ...
        },
        {
          type: 'button',
          span: 3,
          options: [{ label: '搜索', click: this.onSalesSearch, style: 'primary' }]
        }
      ],
      categorieList: null,
      options: null,
  ...
  methods: {
    ...
    changeImage(item) {
      this.qrcode = true;
      this.editForm = { id: item.id, screenshot: item.screenshot, channel_type: 2 };
    },
    ...
    async onAdd() {
      let b = [
        // { label: '信息来源', prop: 'info_source', type: 'radio', options: [{ label: '快商通' }, { label: '快手' }] },
        {
          label: '快捷填单',
          prop: 'referer_info',
          type: 'textarea',
      ...
      if (data.code) {
        ...
        this.editForm = { ...obj, channel: this.editForm.channel, channel_type: 2 };
    ...
    async autoDistribution(item = null, type = true) {
      // 批量自动分配
      if (!this.id) {
    ...
    async autoDistributiona(item = null, type = true) {
      if (type !== true) {
        this.id = item.id;
      }
    ...
    async handDistribution(item = null, type = true) {
      if (type !== true) {
        this.id = item.id;
      }
      // 批量手动分配
